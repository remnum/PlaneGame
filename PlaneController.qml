    ...
    QtObject {
        ...
        function processInput() {
            ...
            if(forwardSpeed > 4.0)
                ...
                forwardSpeed = 0.2
            updatePosition(controlledObject.right, forwardSpeed, controlledObject.position);

            if (moveRight){
                controlledObject.rotate(-1.0 * rightSpeed,Qt.vector3d(0.0,1.0,0.0),Node.ParentSpace);
                if(controlledObject.eulerRotation.x  >= 30)
                    controlledObject.eulerRotation.x  = 30
                else
            ...
            if (moveDown){
                if(controlledObject.eulerRotation.z  >= 90)
                    controlledObject.eulerRotation.z  = 90
                else
            ...
            if(!moveDown && !moveUp){
                if(controlledObject.eulerRotation.z  < -1.0)
                    controlledObject.eulerRotation.z += 1.0
                else if(controlledObject.eulerRotation.z  >1.0)
                    controlledObject.eulerRotation.z -= 1.0
            ...
            if(!moveLeft && !moveRight){
                if(controlledObject.eulerRotation.x  < -0.4)
                    controlledObject.eulerRotation.x += 0.4
                else if(controlledObject.eulerRotation.x  >0.4)
                    controlledObject.eulerRotation.x -= 0.4
